...
impl VmContext {
    ...
    pub fn ext_regs_store(&mut self) {
        unsafe {
            ...
            asm!("mrs {0}, CNTKCTL_EL1", out(reg) self.cntkctl_el1);
            asm!("mrs {0}, CNTP_CTL_EL0", out(reg) self.cntp_ctl_el0);
            asm!("mrs {0}, SP_EL0", out(reg) self.sp_el0);
            asm!("mrs {0}, SP_EL1", out(reg) self.sp_el1);
            asm!("mrs {0}, ELR_EL1", out(reg) self.elr_el1);
            asm!("mrs {0}, SPSR_EL1", out(reg) self.spsr_el1);
            asm!("mrs {0}, SCTLR_EL1", out(reg) self.sctlr_el1);
            asm!("mrs {0}, CPACR_EL1", out(reg) self.cpacr_el1);
            ...
            asm!("mrs {0}, ESR_EL1", out(reg) self.esr_el1);
            ...
            asm!("mrs {0}, CONTEXTIDR_EL1", out(reg) self.contextidr_el1);
            ...
            asm!("mrs {0}, VPIDR_EL2", out(reg) self.vpidr_el2);
