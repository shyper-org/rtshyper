...
impl Vcpu {
    ...
    pub(super) fn new(vm: Weak<Vm>, vcpu_id: usize, phys_id: usize, config: &VmConfigEntry) -> Arc<Self> {
        ...
        #[cfg(any(feature = "memory-reservation"))]
        let inner = Arc::new_cyclic(|weak| Vcpu {
            inner_const,
            ...
            pmu_event: if config.memory.is_limited() {
                ...
                Some(Arc::new(PmuTimerEvent(weak.clone())))
            } else {
                None
            },
    ...
    pub fn context_vm_restore(self: &Arc<Self>) {
    ...
    fn inject_int_inlist(self: &Arc<Self>) {
...
impl Vcpu {
    pub fn remaining_budget(&self) -> u32 {
        self.reservation.remaining_budget()
    }

    pub fn period(&self) -> u64 {
        self.reservation.period()
    }

    pub fn update_remaining_budget(&self, remaining_budget: u32) {
        self.reservation.update_remaining_budget(remaining_budget);
    }

    pub fn reset_remaining_budget(&self) {
        self.reservation.reset_remaining_budget();
    }

    pub fn supply_budget(&self) {
        self.reservation.supply_budget();
    }

    #[cfg(any(feature = "dynamic-budget"))]
    pub fn budget_try_rescue(&self) -> bool {
        self.reservation.budget_try_rescue()
    }
}
